...
export class WebSocketManager extends EventEmitter {
  ...
  private async onWebSocketConnection(client: WebSocketExt, req: http.IncomingMessage) {
    ...
    client.addListener('message', (data: WebSocket.Data) => {
      logger.debug("on client message: ", data);
    ...
    // new Device(client, 'tcp', client.ip + ":" + req.connection.remotePort);
    clientStatusChangeListeners.forEach((listener) => {
  ...
  public sendUtf(client: WebSocket, message: any, cb?: (err: Error, data?: any) => {}) {
   if (client.readyState === WebSocket.OPEN) {
      message = {
        message_id: `${Date.now()}_${Math.random()}`,
        ...message,
      }
      // message.message_id = `${Date.now()}_${Math.random()}`;
      // const str = Buffer.from(JSON.stringify(message), 'utf-8')
      // const str = Buffer.from(JSON.stringify(message), 'utf-8').toString();
      const str = bufferFromString(JSON.stringify(message).toString(), 'utf8');
      logger.debug(`send utf message -> ${str}`);
      client.send(str, (err: Error) => {
        if (err) {
          logger.error(`send message appear error, message -> ${err.message}`);
          cb(err);
        } else {
